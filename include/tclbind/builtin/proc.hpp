...
#ifndef TCLBIND_PROC_HPP
#define TCLBIND_PROC_HPP
...
struct proc
{
  explicit proc(Tcl_Interp * interp, tclbind::object_ptr obj) : interp_(interp), obj_(std::move(obj))
  {
  ...
  const tclbind::object_ptr & object() const {return obj_;}

  int flags() const {return flags_;}
  void set_flags(int flags) {flags_= flags;}
 private:
  Tcl_Interp * interp_;
  tclbind::object_ptr obj_;
  int flags_ = 0;
};
