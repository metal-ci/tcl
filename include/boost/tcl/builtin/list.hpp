...
#ifndef BOOST_TCL_LIST_HPP
#define BOOST_TCL_LIST_HPP
...
inline std::optional<span<Tcl_Obj*>> tag_invoke(
        cast_tag<span<Tcl_Obj*>>,
        Tcl_Interp * interp,
        boost::intrusive_ptr<Tcl_Obj> val)
{
    ...
    return span<Tcl_Obj*>(res, sz);
}

inline object_ptr tag_invoke(const struct convert_tag &, Tcl_Interp*, span<Tcl_Obj*> sv)
{
    return Tcl_NewListObj(sv.size(), sv.data());
}

inline bool tag_invoke(
        const equal_type_tag<span<Tcl_Obj*>> & tag,
        const Tcl_ObjType & type)
{
    return type.name && type.name == string_view("list");
}
        ...
        decltype(tag_invoke(cast_tag<typename Container::value_type>{}, interp, val))* = nullptr)
{
    ...
    for (auto s : span<Tcl_Obj*>(res, sz))
    {
