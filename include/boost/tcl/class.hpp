...
#define BOOST_TCL_DESCRIBE_CONSTRUCTORS_STEP(r, data, elem) \
        BOOST_PP_COMMA_IF( BOOST_PP_SUB ( r , 1)) data(*) elem

#define BOOST_TCL_DESCRIBE_CONSTRUCTORS(Type, ...) \
auto tag_invoke(boost::tcl::detail::get_constructors_tag<Type>) -> \
    boost::mp11::mp_list< BOOST_PP_SEQ_FOR_EACH(BOOST_TCL_DESCRIBE_CONSTRUCTORS_STEP, Type, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__)) >;
...
auto tag_invoke(boost::tcl::detail::get_class_name_tag<Type>) -> core::string_view {return #Name;}
...
struct constructor_call_equal
{
  ...
  void call_impl(Class (*)(Args...), std::index_sequence<Idx...>)
  {
    ...
    const bool all_equal = (is_equal_type<mp11::mp_at_c<types, Idx>>(interp, objv[Idx]) && ...);
    if (all_equal)
        res = new Class(*try_cast<mp11::mp_at_c<types, Idx>>(interp, objv[Idx])...);
  }
...
struct constructor_call_equivalent
{
  ...
  {
    using types = mp11::mp_list<Args...>;
    const bool all_equal = (is_equivalent_type<mp11::mp_at_c<types, Idx>>(objv[Idx]->typePtr) && ...);
    if (all_equal)
      res = new Class(*try_cast<mp11::mp_at_c<types, Idx>>(interp, objv[Idx])...);
  }
...
struct constructor_call_castable
{
  ...
  void call_impl(Class (*)(Args...), std::index_sequence<Idx...>)
  {
    ...
    const auto opts = std::make_tuple(
        try_cast_without_implicit_string<mp11::mp_at_c<types, Idx>>(interp, objv[Idx])...
        );
...
struct constructor_call_with_string
{
  ...
  void call_impl(Class (*)(Args...), std::index_sequence<Idx...>)
  {
    using types = mp11::mp_list<Args...>;
    const auto opts = std::make_tuple(
        try_cast<mp11::mp_at_c<types, Idx>>(interp, objv[Idx])...
    );
...
try
{
  ...
  {
    auto obj = objv[0];
    core::string_view nm{obj->bytes, static_cast<std::size_t>(obj->length)};
    if (nm ==  "boost::tcl::constructor::helper")
    {
  ...
  objv += skip;
  if (res == nullptr)
    mp11::mp_for_each<ctors>(constructor_call_equal{interp, objc, objv, res});
  if (res == nullptr)
    mp11::mp_for_each<ctors>(constructor_call_equivalent{interp, objc, objv, res});
  if (res == nullptr)
    mp11::mp_for_each<ctors>(constructor_call_castable{interp, objc, objv, res});
  if (res == nullptr)
    mp11::mp_for_each<ctors>(constructor_call_with_string{interp, objc, objv, res});
...
struct method_call_equal
{
                  ...
                  std::true_type /* is void */, std::index_sequence<Idx...> )
  {
    ...
    if (all_equal)
    {
      (this_->*Descriptor::pointer)(*try_cast<mp11::mp_at_c<Types, Idx + 1>>(interp, objv[Idx])...);
      done = true;
    }
  }
                 ...
                 std::false_type /* is void */ , std::index_sequence<Idx...> )
  {
    ...
    if (all_equal)
    {
      auto res = (this_->*Descriptor::pointer)(*try_cast<mp11::mp_at_c<Types, Idx + 1>>(interp, objv[Idx])...);
  ...
  {
    using args_t = callable_traits::args_t<decltype(Descriptor::pointer)>;
    if (((std::tuple_size<args_t>::value - 1) != objc) || done || (name != Descriptor::name))
      return;
    call_impl(descr, static_cast<args_t*>(nullptr),
              std::is_void<callable_traits::return_type_t<decltype(Descriptor::pointer)>>{},
...
struct method_call_equivalent
{
  ...
  void call_impl(const Descriptor & descr, Types *,
                 std::true_type /* is void */ , std::index_sequence<Idx...>)
  {
    const bool all_equal = (is_equivalent_type<mp11::mp_at_c<Types, Idx + 1>>(objv[Idx]->typePtr) && ...);
    if (all_equal)
    {
      (this_->*Descriptor::pointer)(*try_cast<mp11::mp_at_c<Types, Idx + 1>>(interp, objv[Idx])...);
      done = true;
...
struct method_call_castable
{
  ...
  {
    const auto opts = std::make_tuple(
        try_cast_without_implicit_string<mp11::mp_at_c<Types, Idx + 1>>(interp, objv[Idx])...
    );
...
struct method_call_with_string
{
  ...
  {
    const auto opts = std::make_tuple(
        try_cast<mp11::mp_at_c<Types, Idx + 1>>(interp, objv[Idx])...
    );
...
try
{
  using descriptor = describe::describe_members<T,
                      describe::mod_inherited | describe::mod_function | describe::mod_public >;
  ...
  const core::string_view name{meth->bytes, static_cast<std::size_t>(meth->length)};

  bool done = false;
  mp11::mp_for_each<descriptor>(method_call_equal<T   >     {interp, objc, objv, done, this_, name});
  if (!done)
    mp11::mp_for_each<descriptor>(method_call_equivalent<T> {interp, objc, objv, done, this_, name});
  if (!done)
    mp11::mp_for_each<descriptor>(method_call_castable<T>   {interp, objc, objv, done, this_, name});
  if (!done)
    mp11::mp_for_each<descriptor>(method_call_with_string<T>{interp, objc, objv, done, this_, name});
...
try
{
  ...
  using descriptors = describe::describe_members<T,
                    describe::mod_inherited | describe::mod_function |
                    describe::mod_public | describe::mod_static >;

  int res = TCL_CONTINUE;
  mp11::mp_for_each<descriptors>(
      [&](auto descr)
      {
...
Tcl_Class register_class(Tcl_Interp * interp)
{
  ...
  using methods = describe::describe_members<T,
      describe::mod_inherited | describe::mod_function | describe::mod_public >;

  std::array<const char*, mp11::mp_size<methods>::value> method_names;
  auto itr = method_names.begin();

  mp11::mp_for_each<methods>(
      [&](auto desc)
      {
        auto it = std::find_if(
  ...
  using static_methods = describe::describe_members<T,
      describe::mod_inherited | describe::mod_function |
      describe::mod_public | describe::mod_static >;

  std::array<const char*, mp11::mp_size<static_methods>::value> static_method_names;
  itr = static_method_names.begin();
  mp11::mp_for_each<static_methods>(
      [&](auto desc)
      {
        auto it = std::find_if(
...
auto tag_invoke(
      ...
      -> std::enable_if_t<describe::has_describe_members<T>::value, T> *
{
...
auto tag_invoke(
    ...
    Tcl_Interp * interp,
    Tcl_Obj * val)
    -> std::enable_if_t<describe::has_describe_members<T>::value, bool>
{
...
inline auto tag_invoke(const convert_tag &, Tcl_Interp* interp, T && t)
    -> std::enable_if_t<describe::has_describe_members<T>::value, object_ptr>
{
  auto cl = register_class<std::decay_t<T>>(interp);

  auto cl_name = tag_invoke(detail::get_class_name_tag<T>{});
  static char internalConstructorMarker[] = "boost::tcl::constructor::helper";
  Tcl_Obj objv[1] = {
