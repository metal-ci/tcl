...
#ifndef METAL_TCL_COMMAND_HPP
#define METAL_TCL_COMMAND_HPP
...
struct sub_command
{
    ...
    sub_command & add_subcommand(boost::core::string_view sv)
    {
        return sub_commands_[std::string(sv)];
    }
  ...
  protected:
    int invoke_(Tcl_Interp * interp, int objc, Tcl_Obj * const objv[])
    {
        if (objc > 1 &&
            (!objv[1]->typePtr ||
              objv[1]->typePtr->name == boost::core::string_view("string")))
        {
        ...
        try {
            ...
            {
                auto candidates = boost::adaptors::filter(overloads_,
...
struct command : sub_command
{
    ...
    Tcl_CmdInfo info(boost::system::error_code & ec)
    {
        Tcl_CmdInfo info_;
